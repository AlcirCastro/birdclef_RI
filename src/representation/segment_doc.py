"""Each segment is a document. No aggregation needed."""
...
@REPRESENTATIONS.register("segment")
class SegmentDocumentStore(BaseDocumentStore):
    name = "segment"
    ...
    def _vector_store_path(self) -> Path:
        if self.cache_dir is None:
            raise ValueError("SegmentDocumentStore needs cache_dir for low-memory build")
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        return self.cache_dir / "segment_vectors.npy"
    ...
    def build(self, items: Sequence[RecordEmbeddings]) -> List[Document]:
        ...
        for record, emb_source in entries:
            mat = load_record_embeddings(emb_source)
            for seg_idx in range(mat.shape[0]):
                vec = mat[seg_idx].astype(np.float32)
                vectors[row] = vec
                self._docs.append(
                    Document(
                        doc_id=row,
