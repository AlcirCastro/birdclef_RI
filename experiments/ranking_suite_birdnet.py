"""BirdNET advanced ranking suite.

Mirror of ranking_suite_advanced.py but defaults to BirdNET v3 configs.
Computes genus@5, genus@10, common@5, common@10 the same way.

Usage:
    python experiments/ranking_suite_birdnet.py
    python experiments/ranking_suite_birdnet.py --out-name ranking_suite_birdnet_custom
    python experiments/ranking_suite_birdnet.py --configs configs/strategy1_segments_birdnet_v3.yaml
"""
...
DEFAULT_CONFIGS = [
    str(ROOT / "configs" / "strategy1_segments_birdnet_v3.yaml"),
    str(ROOT / "configs" / "strategy2_super_embedding_birdnet_v3.yaml"),
]
...
def _evaluate_ranker(
    ...
    if hasattr(ranker, "set_label_meta"):
        ranker.set_label_meta(
            {doc.label: taxonomy_info(doc.scientific_name, doc.common_name) for doc in runner.doc_store.documents}
        )
    ...
    for rec, emb_source in test_emb:
        ...
        for cutoff in genus_hit_at:
            ...
            genus_hit_at[cutoff].append(
                1.0
                if any(
                    genus_from_scientific_name(record_meta[p.label].scientific_name) == true_genus
                    for p in head
                    if p.label in record_meta
                )
                else 0.0
            )
            common_overlap_at[cutoff].append(
                1.0
                if any(
                    true_tokens.intersection(
                        {t for t in record_meta[p.label].common_name.lower().split() if t}
                    )
                    for p in head
                    if p.label in record_meta
                )
...
def _evaluate_ensemble(
    ...
    for rec, emb_source in test_emb:
        ...
        per_query_hits = _build_per_query_hits(
            runner.index, runner.doc_store, runner.fusion, emb, top_k=top_k
        )
        with timer.measure():
    ...
    return {
        ...
        "params": {
            "weights": [
                {"weight": weight, "ranker": ranker_name, "params": ranker_params}
                for weight, ranker_name, ranker_params in ensemble_specs
            ]
        },
        "experiment": runner.cfg.name,
...
def main() -> int:
    ap = argparse.ArgumentParser(description="BirdNET v3 advanced ranking suite with taxonomic metrics")
    ap.add_argument(
        "--configs",
        nargs="+",
    ...
    ap.add_argument("--out-name", default="ranking_suite_birdnet")
    args = ap.parse_args()
    ...
    for run in runs:
        clean = run["clean"]
        lines.append(
            f"| {run['experiment']} | {run['ranker']} "
            f"| {clean.get('MAP', 0.0):.4f} | {clean.get('MRR', 0.0):.4f} "
            f"| {clean.get('P@1', 0.0):.4f} | {clean.get('P@5', 0.0):.4f} "
            f"| {clean.get('nDCG', 0.0):.4f} "
            f"| {clean.get('genus@5', 0.0):.4f} | {clean.get('genus@10', 0.0):.4f} "
            f"| {clean.get('common@5', 0.0):.4f} | {clean.get('common@10', 0.0):.4f} "
