...
\subsection{Métricas Clássicas de Recuperação de Informação}
\label{sub:classicas_fund}

A avaliação de sistemas de busca exige medidas que vão além de simples acertos, utilizando conjuntos de documentos relevantes identificados por especialistas e resultados respondidos pelo sistema. Para fundamentar a avaliação do modelo, adotamos as métricas escalares clássicas detalhadas a seguir.

\paragraph{Mean Average Precision (MAP).} A precisão média nos relevantes encontrados (MAP) é uma medida escalar de precisão fundamental. Para o seu cálculo, extrai-se a média das precisões nos pontos de revocação onde apareceram documentos relevantes. Essa métrica permite a avaliação do sistema a partir de um único valor que resume a qualidade do ranking para múltiplas consultas.

\paragraph{Mean Reciprocal Rank (MRR).} O MRR avalia o desempenho focando especificamente na posição da resposta correta mais alta no ranking \cite{voorhees1999trec}. O cálculo é definido formalmente por:

\begin{equation}
MRR(S, Q) =
\frac{\displaystyle \sum_{q \in Q}
\frac{1}{\mathrm{PosRel}(S(q))}}
{|Q|}
\label{eq:mrr}
\end{equation}

onde $Q$ é um conjunto de consultas, $S$ é um sistema de ranking e $\mathrm{PosRel}(S(q))$ representa a posição do primeiro documento relevante retornado pelo sistema $S$ para a consulta $q$. O MRR (Mean Reciprocal Rank) calcula a média dos inversos dessas posições ao longo de todas as consultas. Em avaliações com profundidade limitada, como o MRR@10, caso o primeiro documento relevante não seja recuperado entre as dez primeiras posições, a contribuição daquela consulta para o cálculo da métrica é igual a zero.

\paragraph{Normalized Discounted Cumulative Gain (nDCG).} Formalizado por Järvelin e Kekäläinen \cite{jarvelin2002cumulated}, o nDCG (Ganho Cumulativo Descontado Normalizado) parte do pressuposto prático de que documentos relevantes perdem utilidade à medida que são apresentados mais longe do topo da resposta. A métrica aplica um fator de desconto logarítmico ao ganho de acordo com a posição do documento. Utilizando relevância linear, a métrica base de Ganho Cumulativo Descontado (DCG) nos $p$ primeiros resultados é dada por:
\begin{equation}
  \text{DCG@}p = \sum_{i=1}^{p} \frac{rel(i)}{\log_2(i+1)}
  \label{eq:dcg}
\end{equation}
Um valor ideal de DCG (IDCG) é obtido caso um sistema coloque todos os documentos ordenados de forma puramente decrescente por valor de relevância. O nDCG divide o DCG de cada sistema por esse DCG ideal, resultando em valores normalizados no intervalo $[0, 1]$ para cada posição do ranking.
...
\begin{table}[H]
\centering
\caption{Resultados do sweep de rerankers com BirdNET v3.0 (6.835 consultas).
Melhores por estratégia em \textbf{negrito}.}
\label{tab:sweep_birdnet}
\begin{tabular}{llccccc}
\toprule
\textbf{Est.} & \textbf{Ranker} & \textbf{MAP} & \textbf{nDCG} & \textbf{P@1} & \textbf{genus@5} & \textbf{Lat.} \\
\midrule
E1 & \texttt{attention} ($\tau=0{,}05$)   & \textbf{0,8942} & \textbf{0,9103} & \textbf{0,8549} & 0,9476 & 11,8~ms \\
E1 & \texttt{softmax} ($\tau=0{,}1$)      & 0,8922 & 0,9085 & 0,8524 & 0,9473 & 12,5~ms \\
E1 & \texttt{hybrid\_att\_sm\_tax}         & 0,8923 & 0,9089 & 0,8521 & \textbf{0,9472} & \textbf{0,9~ms} \\
E1 & \texttt{taxonomy\_boost}             & 0,8892 & 0,9062 & 0,8477 & 0,9454 & 11,7~ms \\
E1 & \texttt{hybrid\_att\_rrf\_tax}        & 0,8881 & 0,9052 & 0,8483 & 0,9400 & 0,8~ms \\
E1 & \texttt{rrf} ($k=60$)                & 0,8319 & 0,8563 & 0,7782 & 0,9080 & 11,4~ms \\
E1 & \texttt{borda}                        & 0,8284 & 0,8537 & 0,7722 & 0,9074 & 11,5~ms \\
E1 & \texttt{topk\_mean}                   & 0,7708 & 0,8004 & 0,7080 & 0,8612 & 11,8~ms \\
\midrule
E2 & \texttt{attention} ($\tau=0{,}05$)   & \textbf{0,8373} & \textbf{0,8604} & \textbf{0,7881} & \textbf{0,9086} & 5,1~ms \\
E2 & \texttt{hybrid\_att\_rrf\_tax}        & 0,8404 & 0,8623 & 0,7921 & 0,9105 & \textbf{0,3~ms} \\
E2 & \texttt{hybrid\_att\_sm\_tax}         & 0,8320 & 0,8562 & 0,7807 & 0,9048 & \textbf{0,3~ms} \\
E2 & \texttt{softmax}                      & 0,8298 & 0,8540 & 0,7786 & 0,9033 & 5,1~ms \\
E2 & \texttt{taxonomy\_boost}             & 0,8240 & 0,8493 & 0,7700 & 0,9007 & 5,1~ms \\
E2 & \texttt{rrf}                          & 0,8108 & 0,8371 & 0,7557 & 0,8879 & 5,0~ms \\
\bottomrule
\end{tabular}
\end{table}

Com BirdNET, o \texttt{attention} em E1 alcança MAP\,=\,0,8944 --- o
\textbf{melhor resultado de todo o projeto} (Figura~\ref{fig:map_bars}),
...
\begin{axis}[
  ...
  \addplot[fill=azulRI!45, draw=azulRI!70] coordinates {
    (0.252, 0)
    (0.763, 1)
    (0.811, 2)
    (0.818, 3)
...
\end{axis}
\end{tikzpicture}
\caption{MAP dos principais experimentos. PV\,=\,Perch v2,
BN\,=\,BirdNET v3.0. O BirdNET com \texttt{attention} em E1 atinge o melhor
MAP (0,894).}
\label{fig:map_bars}
\end{figure}
...
\begin{table}[ht]
\centering
\caption{Métricas taxonômicas em E1 (6.835 consultas) para Perch v2 e BirdNET v3.0.
genus@k: fração dos top-$k$ do mesmo gênero; common@10: fração dos top-10
com nome comum sobreposto. Melhores por modelo em \textbf{negrito}.}
\label{tab:tax}
\begin{tabular}{llcccc}
\toprule
\textbf{Modelo} & \textbf{Reranker} & \textbf{MAP} & \textbf{genus@5} & \textbf{genus@10} & \textbf{common@10} \\
\midrule
Perch v2 & \texttt{softmax}              & 0,8682 & \textbf{0,9242} & 0,9419 & 0,9486 \\
Perch v2 & \texttt{attention}            & \textbf{0,8689} & 0,9223 & \textbf{0,9424} & \textbf{0,9492} \\
Perch v2 & \texttt{hybrid\_att\_sm\_tax} & 0,8691 & 0,9247 & 0,9431 & 0,9498 \\
Perch v2 & \texttt{taxonomy\_boost}      & 0,8628 & 0,9207 & 0,9409 & 0,9482 \\
Perch v2 & \texttt{rrf}                  & 0,8179 & 0,8844 & 0,9154 & 0,9254 \\
Perch v2 & \texttt{borda}                & 0,8170 & 0,8832 & 0,9175 & 0,9271 \\
\midrule
BirdNET  & \texttt{softmax}              & 0,8922 & 0,9473 & 0,9624 & 0,9684 \\
BirdNET  & \texttt{attention}            & \textbf{0,8942} & \textbf{0,9476} & \textbf{0,9630} & \textbf{0,9688} \\
BirdNET  & \texttt{hybrid\_att\_sm\_tax} & 0,8923 & 0,9472 & \textbf{0,9630} & \textbf{0,9688} \\
BirdNET  & \texttt{taxonomy\_boost}      & 0,8892 & 0,9454 & 0,9618 & 0,9680 \\
BirdNET  & \texttt{rrf}                  & 0,8319 & 0,9080 & 0,9362 & 0,9462 \\
BirdNET  & \texttt{borda}                & 0,8284 & 0,9074 & 0,9364 & 0,9459 \\
\bottomrule
\end{tabular}
\end{table}
...
Este trabalho apresentou o \emph{BirdSearch}, um sistema modular de recuperação de informação acústica projetado para a identificação automatizada de espécies de aves e validado sob o expressivo benchmark BirdCLEF 2026. A pesquisa promoveu uma investigação sistemática sobre o impacto de duas estratégias de estruturação de índices e fusão de evidências — a fusão tardia baseada em segmentos (E1) e a fusão precoce via super-embeddings gerados por \emph{max-pooling} (E2) —, operando em conjunto com 14 algoritmos de ranqueamento sobre os backbones neurais especializados Google Perch v2 e BirdNET v3.0.

Os resultados experimentais evidenciaram que a fusão precoce (E2) consolida-se como a abordagem mais eficiente em termos computacionais, reduzindo a latência média de busca para 6,3 ms (um ganho de velocidade de 3,6$\times$ em relação a E1) mantendo patamares competitivos de MAP (0,8227). Em contrapartida, a fusão tardia (E1) demonstrou clara vantagem na preservação da diversidade de candidatos na lista expandida, alcançando maior precisão no corte de top-5 (P@5 = 0,2518). No âmbito do pós-ranqueamento, os mecanismos baseados em normalização de scores absolutos (\emph{attention} e \emph{softmax}) superaram de forma consistente as tradicionais técnicas de fusão por ordens de rank (RRF e Borda Count), registrando o ápice de eficácia do sistema com MAP de 0,8944 através do arranjo BirdNET + \emph{attention} em E1. Por fim, a análise do impacto dos componentes validou o caráter indispensável da segmentação temporal com 50\% de sobreposição (\emph{hop size}), cuja ausência acarreta uma degradação severa de até 3 pontos no MAP.

A modularidade do ecossistema do \emph{BirdSearch} abre caminhos para extensões promissoras. Como trabalhos futuros, identificam-se as seguintes diretrizes de pesquisa:
\begin{itemize}
    \item Investigar a formulação de filtros de ruído alternativos que atenuem interferências ambientais de campo sem descaracterizar as pistas espectrais discriminativas aprendidas nativamente pelo modelo Perch v2;
    \item Explorar o ajuste fino (\emph{fine-tuning}) dos backbones neurais empregados, focando especificamente no mapeamento de vocalizações de espécies endêmicas da região amazônica;
    \item Avaliar a adoção de representações estruturadas em agrupamentos (\emph{cluster}) e protótipos vetoriais, visando modelar de maneira mais fiel os repertórios vocais multi-modais complexos das aves;
    \item Desenvolver e implementar estruturas de indexação dinâmica com o propósito de conferir suporte escalável e de baixa latência a corpora de monitoramento bioacústico contínuo.
\end{itemize}
